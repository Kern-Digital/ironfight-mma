 ...
 * components/motion — der Aufrufer rendert das Sheet immer und meldet „zu"
 * über `session={null}`, damit auch das Schließen eine Bewegung hat.
 */
...
export default function WorkoutLogSheet({
  ...
  // Während der Austritts-Feder ist `session` schon null; ohne den letzten
  // Wert führe ein leeres Panel hinaus.
  const zeigen = useLetzterWert(session);

  return (
    <SheetShell
      open={session !== null}
      onClose={onClose}
      label={`Details zu „${zeigen?.label ?? "Workout"}"`}
      zIndex={zIndex}
